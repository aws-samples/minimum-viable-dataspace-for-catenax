...
The MVD's EDC connectors come with API key-based authentication that is configured as part of this project's [`deploy.sh`](https://github.com/aws-samples/minimum-viable-dataspace-on-aws/blob/main/deploy.sh) dialog. This API key then needs to be provided through an HTTP header `x-api-key` with each invocation of one of the EDCs' APIs.
